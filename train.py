...
#根据标定的输入数据获取images, origin_images, labels
...
#补充获取8s特征图,即获取vgg_16/pool3节点的输出,大小为: batchsize x 28 x 28 x 256
pool3_feature = end_points['vgg_16/pool3']
#对vgg_16/pool3节点的输出进行1x1卷积
...
#获取vgg16中所获得的pool4的输出,形状大小应该为:batchsize x 14 x 14 x 512
pool4_feature = end_points['vgg_16/pool4']
#这里会获取vgg16/fc8的输出(分类结果)分类结果判定为True时,则获取16s
#16s基于pool4网络的输出14x14x512进行一个1x1xclasses(1x1x21)的卷积,不进行激活,并使用0来初始化该层卷积核
#这里采用了pool4的网络结果进行运算,实际上在vgg16网络中,这里将原本224大小的图片缩小到了1/16,所以是16s的logits
with tf.variable_scope('vgg_16/fc8'):
...
#准备进行上采样,初始化上采样所使用的权重矩阵,这里获取的矩阵为2x2xx21的初始值为后两个维度的值为0.25的思维权重矩阵
...
#卷积的反向运算,长宽步长为2,即长宽会扩大2倍
upsampled_logits_l = tf.nn.conv2d_transpose(logits, upsample_filter_tensor_x2,
